...
RUN NB_CORES=${BUILD_CORES-$(getconf _NPROCESSORS_CONF)} \
  ...
  && build_pkgs="build-base subversion git libtool automake autoconf tar xz binutils curl-dev cppunit-dev libressl-dev zlib-dev linux-headers ncurses-dev ncurses-libs libxml2-dev" \
...
# compile xmlrpc-c
  ...
  && cd xmlrpc-c \
  && ./configure --disable-cgi-server --disable-libwww-client --disable-wininet-client \
  && make -j ${NB_CORES} \
  && make install \
